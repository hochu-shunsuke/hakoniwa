...
/** 16 方位の単位ベクトル（cos, sin を定数で持つ）。 */
const WIND_DIRECTIONS: readonly (readonly [number, number])[] = [
  [1, 0], [0.92388, 0.382683], [0.707107, 0.707107], [0.382683, 0.92388],
  [0, 1], [-0.382683, 0.92388], [-0.707107, 0.707107], [-0.92388, 0.382683],
  [-1, 0], [-0.92388, -0.382683], [-0.707107, -0.707107], [-0.382683, -0.92388],
  [0, -1], [0.382683, -0.92388], [0.707107, -0.707107], [0.92388, -0.382683],
];
...
export class Climate {
  ...
  constructor(a: number, b: number, c: number, d: number, massAt: MassAt) {
    ...
    // 16 方位から選ぶ。Math.cos / sin は JS エンジンごとに結果がわずかに違いうるので、
    // 雨陰 → 雨量 → 川の流れの計算に入る値には使わない（CLAUDE.md の決定性の決まり）。
    const [wx, wz] = WIND_DIRECTIONS[((a ^ 0x1b873593) >>> 0) % WIND_DIRECTIONS.length];
    this.windX = wx;
    this.windZ = wz;
  }

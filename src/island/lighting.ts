/**
 * 島全体の光を焼き込む。太陽の影（山が落とす影）と、空の見え方（谷や峡谷ほど空が狭く暗い）。
 *
 * 島は有限で、太陽は動かない。だから光は島を作るたびに 1 度だけ計算し、描くときは
 * テクスチャを 1 回引くだけにする（render/islandLight.ts）。影をその場で計算する
 * シャドウマップ（毎フレームもう 1 度描く）より軽く、島の端から端まで届く長い影が落ちる。
 *
...
export interface IslandLighting {
  /** 1 辺の点数（高さの格子と同じ）。 */
  n: number;
